...
class WeaviateWCS:
    ...
    def __init__(self, 
        ...
        if self.model_name_or_path == 'text-embedding-3-small':
        ...
        self.return_properties = ['file_name', 'page_label', 'document_title', 'page_summary', 'page_url', 'doc_id', \
                                    'content']
    ...
    def get_openai_embedding(self, query: str) -> list[float]:
        ...
        '''
        embedding = self.model.embeddings.create(input=query, model='text-embedding-3-small', dimensions=256).model_dump()
        if embedding:
